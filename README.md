...
- プロンプトの原本は `prompts/` 配下の Markdown ファイルです（1ファイル＝1プロンプト）。
...
- **`## プロンプト` の次の行からファイル末尾までが「コピー」の対象**です。
  ここには生成AIへ実際に入力する文面だけを書きます。
- そのため、**`## 使い方` は必ず `## プロンプト` より前に置いてください。**
  `## プロンプト` より後に書いた見出しは、すべてプロンプトの一部としてコピーされます。
- 逆に言えば、プロンプト自体を見出しで構造化しても問題ありません。
  次のように書けば、`## 前提条件` 以降も含めて丸ごとコピーされます。
